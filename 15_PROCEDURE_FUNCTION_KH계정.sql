/*
    ...
    필요할 때마다 내가 작성한 PL/SQL문을 편하게 호출가능하다.
    ...
    EXEC 프로시져명;
*/
-- EMPLOYEE 테이블을 보가한 COPY 테이블 만들기
...
/*
    ...
    DB에서 처리하는게 당연히 성능이 더 좋음.(데이터를 넘길 때 마다 네트워크 자원들 소비하게됨)
    
    프로시져 단점
    1. DB자원을 직접 사용하기 때문에 DB에 부하를 주게됨(남용하면 안된다.)
    2. 관리적인 측면에서 자바소스코드, 오라클 코드를 동시에 형상관리하기가 어려움.
...
RETURN VARCHAR2
IS
    RESULT VARCHAR2(1000); -- BEGIN 문안에서 사용할 RESULT 값 셋팅
BEGIN
