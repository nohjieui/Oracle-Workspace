...
/*
    ...
    - CHAR(바이트수) : 최대 2000BYTE까지 지정가능.
                      고정길이(아무리 적은 값이 들어와도 빈공간은 공백으로 채워 처음 할당한 크기를 유지함)
      ...
      VARCHAR2(바이트수/CHAR 크기) : 최대길이 4000BYTE까지 가능
                          ...
                          VARCHAR2(CHAR 10)
      NVARCHAR : 문자열의 바이트가 아닌, 문자 갯수 자체를 길이로 취급하여 유니코드를 지원하기 위한 자료형 ★시험에 나옴★
...
CREATE TABLE MEMBER( -- 동일한 테이블명 중복 불가.
    MEMBER_ID VARCHAR2(20), -- 대소문자 구분하지 않음.  따라서 낙타등표기법 의미 없다. => 언더바로 구분.
...
INSERT INTO MEMBER VALUES('user01', 'pass01' , '노지의' , '1995-02-08');
...
/*
    ...
    - 원하는 데이터값만 유지하기 위해서 특정 칼럼마다 설정하는 제약.
...
INSERT INTO MEM_UNIQUE VALUES(3,'user01', 'pass03', '노지의3'); --ORA-00001: 무결성 제약 조건(DDL.SYS_C0011109)에 위배됩니다
-- UNIQUE 제약조건에 위배됨.
...
VALUES(1, 'user01', 'pass01', '노지의', 'Z', '010-1111-1111', null, SYSDATE);
-- CHECK 제약조건 위배.
INSERT INTO MEM_CHECK
VALUES(4, 'user04', 'pass01', '노지의4', NULL, '010-1111-1111', null, SYSDATE);
-- CHECK 제약조건에 NULL값도 추가 가능함.
-- NULL값을 추가적으로 못들어오게 하고싶다면? NOT NULL 제약조건을 걸어줘야한다.
...
INSERT INTO MEM_CHECK
VALUES (1, 'user01', 'pass01', '노지의', 'F', NULL, NULL);
...
/*
    ...
    컬럼에 부여하는 제약조건
    => 다른테이블을 참조한다 라고 표현
        즉, 참조된 다른 테이블이 제공하고 있는 값만 해당칼럼에 들어갈 수 있다.
        EX) KH계정에서
    ...
    참조할 테이블 == 부모테이블
    생략가능한것 : CONSTRAINT 제약조건명, 참조할컬럼명
...
CREATE TABLE MEM (
    ...
    EMAIL VARCHAR2(30),
    MEM_DATE DATE DEFAULT SYSDATE NOT NULL -- DEFAULT 설정을 먼저 하고나서 제약조건을 추가해야함
    -- ,FOREIGN KEY(GRADE_ID) REFERENCES MEM_GRADE(GRADE_CODE) -- 테이블레벨방식 외래키 지정
);

INSERT INTO MEM(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, GRADE_ID, GENDER)
VALUES (1, 'user01', 'pass01', '지의', 'G1', 'F');

INSERT INTO MEM(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, GRADE_ID, GENDER)
VALUES (2, 'user02', 'pass01', '지의', 'G2', 'F');

INSERT INTO MEM(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, GRADE_ID, GENDER)
VALUES (3, 'user03', 'pass01', '지의', 'G3', 'F');

INSERT INTO MEM(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, GRADE_ID, GENDER)
VALUES (4, 'user04', 'pass01', '지의', NULL, 'F');
...
-- 외래키 제약조건 부여시 삭제에대한 옵션을 추가해주면 됨.
-- => 기본적으로 삭제제한옵션이 있음.
...
/*
    ...
    - ON DELETE CASCADE :  부모데이터를 삭제할 때 해당 데이터값을 사용하는 자식데이터를 함께 삭제하겠다.
...
CREATE TABLE MEM (
    ...
    EMAIL VARCHAR2(30),
    MEM_DATE DATE DEFAULT SYSDATE NOT NULL -- DEFAULT 설정을 먼저 하고나서 제약조건을 추가해야함
    ,FOREIGN KEY(GRADE_ID) REFERENCES MEM_GRADE(GRADE_CODE) ON DELETE SET NULL -- 테이블레벨방식 외래키 지정
);
...
CREATE TABLE MEM (
    ...
    EMAIL VARCHAR2(30),
    MEM_DATE DATE DEFAULT SYSDATE NOT NULL -- DEFAULT 설정을 먼저 하고나서 제약조건을 추가해야함
    ,FOREIGN KEY(GRADE_ID) REFERENCES MEM_GRADE(GRADE_CODE) ON DELETE CASCADE -- 테이블레벨방식 외래키 지정
);
...
-- 전체사원들 중 급여가 300만원 이상인 사번들의 사번, 이름, 부서코드, 급여 칼럼 복제하시오(내용물포함)
...
AS SELECT EMP_ID, EMP_NAME, SALARY, SALARY * 12 AS "연봉"
   FROM EMPLOYEE;
-- must name this expression with a column alias : 열의 별명과 함께 지정해야 합니다
