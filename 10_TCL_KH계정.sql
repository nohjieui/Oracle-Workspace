/*
    ...
    * 트랜잭션(Transaction)
    ...
    - 데이터의 변경사항(DML)들을 하나의 트랜잭션으로 묶어서 처리
      => COMMIT(확정,승인)하기 전까지 변경사항들은 모두 묶어서 하나의 트랜잭션으로 담음.
    ...
    * 트랜잭션의 종류
    ...
    - ROLLBACK TO 포인트명 : 전체 변경사항을 삭제하는 것이 아니라, SAVE포인트로 저장해놓은 시점으로 롤백함
*/
...
/*
    ...
    기존에 트랜잭션에 있던 모든 변경사항들을 무조건 실제 DB에반영(COMMIT) 시킨 후에 DDL이 수행됨.
