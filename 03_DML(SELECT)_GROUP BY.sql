...
FROM EMPLOYEE -- 1. FROM절 제일 먼저 실행
...
FROM EMPLOYEE
WHERE  ROUND(AVG(SALARY)) >= 3000000 --> 오류발생. 문법상 그룹함수를 WHERE절에서 쓸 수 없음
GROUP BY DEPT_CODE;
...
-- 각 직급별로 총 급여의 합이 1000만원 이상인 직급코드, 급여 합을 조회
SELECT JOB_CODE 직급코드, SUM(SALARY) AS "급여의 합"
FROM EMPLOYEE
GROUP BY JOB_CODE
...
/*
    ...
    주의해야할 점 : 두 쿼리문의 결과를 합쳐서 한개의 테이블로 보여줘야 하기 때문에 두 쿼리문의 SELECT절 부분은 같아야한다.
                   즉 조회할 "컬럼"이 동일해야한다.
*/
