...
--> JOIN을 통해서 연결고리에 해당되는 컬럼들만 제대로 매칭시키면 마치 하니의 결과물로 조회가능하다.
/*
    ...
    + 만약에 연결할 칼럼명이 동일하다면 USING구문을 사용하고,
                          일치하지 않다면, 명시적으로 어느테이블로부터 온 칼럼인지 작성해줘야한다.
*/
-- 오라클 전용구문.
...
WHERE DEPT_CODE = DEPT_ID; -- EMPLOYEE테이블의 DEPT_CODE값과 DEPARTMENT테이블의 DEPT_ID이 일치하는 경우에만 조회.
...
-- 방법 2) 테이블에 별칭을 붙여서 사용하는 방법(각 테이블마다 별칭부여 가능) => 별칭.컬럼명
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME
FROM EMPLOYEE E, JOB J --> 테이블에 별칭을 붙일때 AS 키워드 사용 X
...
-- 1) 연결할 칼럼명이 다른경우(EMPLOYEE - DEPT_CODE / DEPARTMENT - DEPT_ID)
--    => 무조건 ON구문만 가능.
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
...
-- 2-1) ON 구문 : ambiguosly가 발생할 수 있기 때문에 칼럼명을 명확하게 기술해줘야함.
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE);

-- 2-2) USING 구문 : 컬럼명이 동일할 경우에만 사용 가능.
--                   동일한 컬럼명 하나만 써주면 알아서 매칭시켜줌.
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE);

-- [참고] 자연조인(NATURAL JOIN) : 등가조인 방법 중 하나
-- => 동일한 타입과 이름을 가진 칼럼을 조인조건으로 이용하는 방법.
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB;
-- 운좋게도 두개의 테이블에 일치하는 컬럼명이 "딱 한개"만 존재(JOB_CODE) --> 자연조건으로 사용시 알아서 JOB_CODE가 매칭됨.
...
--------------------------- 실습문제 --------------------------------------------
-- 1. 부서과 '인사관리부'인 사원들의 사번, 사원명, 보너스를 조회
...
-- 오라클 전용구문, ANSI구문 2가지로 표현
SELECT * FROM DEPARTMENT; -- LOCATION_CODE
...
/*
    ...
    단, 반드시 LEFT / RIGHT를 지정해야함, -> 기준이 되는 테이블을 기정해야 한다.
...
-- 일치하는 행들 + LEFT OUTER JOIN 기준으로 새롭게 추가된 행들 + RIGHT OUTER JOIN
--> ANSI 구문
SELECT EMP_NAME, SALARY, DEPT_TITLE
FROM EMPLOYEE
...
-- 카테시안의 곱의 경우 : WHERE 조건절에 기술하는 조인물이 잘못되었거나 아예 없을 때 발생.

/*
    4. 비등가 조인(NON EQUAL JOIN)
    '=' 를 사용하지 않는 조인문 => 다른 비교연산자를 써서 조인함.( > , < , >=, <= , BETWEEN A AND B)
    => 지정한 칼럼값들이 일치하는 경우가 아니라 "범위"에 포함되는 경우 매칭에 조회하겠다.
...
/*
    ...
    => 자체조인의 경우 테이블에 반드시 별칭을 붙여줘야 한다. (별칭을 붙이면 서로 다른 테이블인 것처럼 사용가능)
*/
