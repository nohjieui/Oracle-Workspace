/*
    SUBQUERY(서브쿼리)
    하나의 주된 SQL(SELECT, CRAETE, UPDATE, INSERT) 안에 포함된 또 하나의 SELECT문
...
/*
    ...
    - 단일행 (단일열) 서브쿼리 : 서브쿼리를 수행한 결과값이 오직 1개일때(한칸의 컬럼값이 나올때)
...
-- 과장직급임에도 불구하고 "모든" 차장직급의 급여보다도 더 많이 받는 직워 조회(사번,이름,직급명,급여)
...
/*
    ...
    서브쿼리 조회 결과가 값은 한행지지만, 나열된 컬럼의 갯수가 여러개인 경우 ..
*/
...
-- (비교대상칼럼1 , 비교대상칼럼 2) = (비교할값 1, 비교할값 2 ) -> 서브쿼리로 제시
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
...
-- *ROWNUM : 오라클에서 자동으로 제공해주는 칼럼. "조회된" 순서대로 1부터 순번을 부여해주는 칼럼.
...
-- 선 정렬 후 순위매기기를 해야한다, => 우선적으로 인라인뷰로 ORDER BY절을 만들고 그 후에 메인쿼리에서 순번을 붙임.
...
/*
    ...
    정렬기준 : ORDER BY절(정렬기준 컬럼이름, 오름차순/내림차순), NULLFIRST나 NULLSLAST는 기술 불가.
    
    오직 SELECT 절에서만 기술 가능.
*/

-- 사원들의 급여가 높은 순서대로 매기기. 사원명, 급여, 순위 조회
-- RANK OVER
SELECT EMP_NAME, SALARY, RANK() OVER(ORDER BY SALARY DESC) "순위"
FROM EMPLOYEE; -- 공동 19위 2명, 그 다음 순위 21위
-- DENSE_RANK OVER
SELECT EMP_NAME, SALARY, DENSE_RANK() OVER(ORDER BY SALARY DESC) "순위"
FROM EMPLOYEE; -- 공동 19위 2명, 그 다음 순위 20위

-- 5위까지만 조회하겠다.
SELECT EMP_NAME, SALARY, DENSE_RANK() OVER(ORDER BY SALARY DESC) "순위"
FROM EMPLOYEE
WHERE DENSE_RANK() OVER(ORDER BY SALARY DESC) <= 5;
-- 윈도우함수를 WHERE절에서 기술 불가능.
